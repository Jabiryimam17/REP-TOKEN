...
export  const sign_up = async (user) =>{
    ...
    const [nonce_rows] = await db.query("SELECT * FROM nonces WHERE nonce = ?", user.nonce);

    if (!nonce_rows || nonce_rows.length === 0 || nonce_rows[0].used ) return false;
    ...
    const [result] = await db.query('INSERT INTO users (email, pass_hash, address, hash_address, role, f_name, l_name, profile_picture) VALUES (?, ?, ?, ?, ?, ?, ?, ?) RETURNING id', [user.email, bcrypt.hashSync(user.password, 10), address, user.hash, user.role, user.f_name, user.l_name, user.profile_picture || null]);
