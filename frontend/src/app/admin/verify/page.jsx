...
export default function AdminVerifyPage() {
    ...
    const getRoleBadge = (role) => {
        switch (role) {
            case "freelancer":
                return <span
                    className="px-2.5 py-0.5 rounded-full bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 text-xs font-bold uppercase flex items-center w-fit"><Briefcase
                    className="w-3 h-3 mr-1"/> Freelancer</span>;
            case "employer":
                return <span
                    className="px-2.5 py-0.5 rounded-full bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 text-xs font-bold uppercase flex items-center w-fit"><User
                    className="w-3 h-3 mr-1"/> Employer</span>;
            case "verifier":
                return <span
                    className="px-2.5 py-0.5 rounded-full bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 text-xs font-bold uppercase flex items-center w-fit"><Scale
                    className="w-3 h-3 mr-1"/> Verifier</span>;
            default:
                return null;
        }
    };
        ...
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950/50 py-12 px-4 sm:px-6 lg:px-8">
            <div className="max-w-7xl mx-auto">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
                    <div>
                        <nav className="flex mb-4 text-sm font-medium text-slate-500 dark:text-slate-400">
                                <span
                                    className="hover:text-indigo-600 transition-colors cursor-pointer">Admin Panel</span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    <div
                        ...
                        <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">Pending
                            Requests</p>
                        <h3 className="text-2xl font-black text-slate-900 dark:text-white">{pendingUsers.length}</h3>
                    </div>
                    <div
                        className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm">
                        <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">Avg. Response
                            Time</p>
                        <h3 className="text-2xl font-black text-indigo-600">4.2 hours</h3>
                    </div>
                    <div
                        className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm">
                        <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">Verified
                            Today</p>
                ...
                <div
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-left border-collapse">
                            ...
                            <tr className="border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
                                <th className="px-6 py-4 text-xs font-bold text-slate-400 uppercase tracking-widest">User
                                    Details
                                </th>
                                <th className="px-6 py-4 text-xs font-bold text-slate-400 uppercase tracking-widest">Role</th>
                                <th className="px-6 py-4 text-xs font-bold text-slate-400 uppercase tracking-widest">Public
                                    Address
                                </th>
                                ...
                                <tr>
                                    <td colSpan="5" className="px-6 py-20 text-center">
                                        <div className="flex flex-col items-center">
                                            <div
                                                className="animate-spin rounded-full h-12 w-12 border-b-2 border-indigo-600 mb-4"></div>
                                            <p className="text-slate-500 dark:text-slate-400 font-medium text-lg">Loading
                                    ...
                                    <tr key={user.id}
                                        ...
                                        <td className="px-6 py-6">
                                            <div className="flex items-center">
                                                <div
                                                    className="w-10 h-10 rounded-full bg-gradient-to-tr from-indigo-500 to-blue-500 flex items-center justify-center text-white font-bold mr-4 shadow-sm">
                                                    {user.name.charAt(0)}
                                                </div>
                                                <div>
                                        ...
                                        <td className="px-6 py-6">
                                            <div className="flex items-center space-x-2 group">
                                                <code
                                                    className="px-2 py-1 bg-slate-50 dark:bg-slate-800 rounded border border-slate-100 dark:border-slate-700 text-xs text-slate-600 dark:text-slate-400">
                                ...
                                <tr>
                                    <td colSpan="5" className="px-6 py-20 text-center">
                                        <div className="flex flex-col items-center">
                                            ...
                                            <p className="text-slate-500 dark:text-slate-400 font-medium text-lg">No
                                                pending verification requests found.</p>
                                            <p className="text-sm text-slate-400">Check back later or try a
                    ...
                    <div
                        className="px-6 py-4 bg-slate-50 dark:bg-slate-900/50 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                        <p className="text-xs text-slate-500 font-medium">Showing {filteredUsers.length} of {pendingUsers.length} requests</p>
                        <div className="flex space-x-2">
                            <button
                                className="px-3 py-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-xs font-bold text-slate-600 dark:text-slate-400 disabled:opacity-50"
                                disabled>Previous
                            </button>
                            <button
                                className="px-3 py-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-xs font-bold text-slate-600 dark:text-slate-400 disabled:opacity-50"
                ...
                {/* Security Guidelines */}
                <div
                    className="mt-12 bg-indigo-50 dark:bg-indigo-900/10 p-8 rounded-3xl border border-indigo-100/50 dark:border-indigo-900/30 flex flex-col md:flex-row items-center gap-8">
                    <div className="p-4 bg-indigo-600 rounded-2xl text-white">
                        <Lock className="w-8 h-8"/>
                    </div>
                    <div>
                        <h4 className="text-lg font-bold text-indigo-900 dark:text-indigo-300 mb-2">Verification
                            Security Protocol</h4>
                        <p className="text-sm text-indigo-700 dark:text-indigo-400 leading-relaxed max-w-3xl">
                            As an administrator, you are handling sensitive personal data. Ensure you
                            cross-reference the photo ID with the user's claimed identity. Verify that the public
                            address matches the signature provided during registration. All actions in this panel
                            are logged for governance auditing.
                        </p>
                    </div>
                    <button
                        className="whitespace-nowrap flex items-center px-6 py-3 bg-white dark:bg-indigo-600 text-indigo-600 dark:text-white rounded-xl font-bold text-sm shadow-sm hover:shadow-md transition-all">
            ...
            {isModalOpen && selectedUser && (
                <div
                    ...
                    <div
                        className="bg-white dark:bg-slate-900 rounded-3xl w-full max-w-4xl max-h-[90vh] overflow-hidden shadow-2xl relative animate-in fade-in zoom-in duration-300">
                        <button
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2">
                            <div
                                className="p-8 border-b md:border-b-0 md:border-r border-slate-100 dark:border-slate-800">
                                <div className="flex items-center mb-6">
                                    <div
                                        className="w-12 h-12 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-600 font-bold mr-4">
                                ...
                                <div className="space-y-6">
                                    <div>
                                        <label
                                            className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-1">Public
                                            Address</label>
                                        <div
                                            className="p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 font-mono text-xs text-slate-600 dark:text-slate-400 break-all leading-relaxed">
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            <label
                                                className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-1">Role</label>
                                            <div
                                                className="font-bold text-slate-700 dark:text-slate-300 capitalize">{selectedUser.role}</div>
                                        </div>
                                        <div>
                                            <label
                                                className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-1">Uploaded</label>
                                            <div
                                    ...
                                    {selectedUser.role === 'verifier' && (
                                        <div>
                                            <label
                                                className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-1">Category</label>
                                            <select
                                    ...
                                    <div
                                        className="pt-6 border-t border-slate-100 dark:border-slate-800 flex gap-4">
                                        <button
                                            onClick={() => {
                            ...
                            <div
                                className="bg-slate-100 dark:bg-slate-800 flex flex-col p-8 items-center justify-center relative">
                                <div className="absolute top-4 left-4 flex space-x-2">
                                    <div
                                        className="px-2 py-1 bg-black/50 backdrop-blur-md rounded-lg text-[10px] font-bold text-white uppercase tracking-widest">Document
                                        Preview
                                    </div>
                                </div>

                                <div
                                    className="w-full h-full min-h-[400px] rounded-2xl overflow-hidden shadow-lg border-4 border-white dark:border-slate-700 relative group">
                                    {selectedUser.idImage ? (
                                        ...
                                        <div
                                            className="w-full h-full flex items-center justify-center bg-slate-200 dark:bg-slate-700 text-slate-400">
                                            <FileText className="w-12 h-12"/>
                                        </div>
                                    )}
                                    <div
                                        className="absolute inset-0 bg-black/20 group-hover:bg-black/0 transition-colors"></div>
                                    <button
                                        className="absolute bottom-4 right-4 p-3 bg-white/90 backdrop-blur-md rounded-xl text-slate-900 shadow-xl opacity-0 group-hover:opacity-100 transition-opacity">
                                        <Maximize2 className="w-5 h-5"/>
                                    </button>
                                </div>

                                <p className="mt-4 text-xs text-slate-500 font-medium flex items-center">
                                    <AlertCircle className="w-3 h-3 mr-1"/> Inspect for tempering or photo
                                    manipulation
                                </p>
                            </div>
                        </div>
